...
if ($res && $res->num_rows > 0) {
    ...
    if (!empty($row['room_id'])) {
        ...
        // Generate new unique room ID and update DB
        $roomID = uniqid("room_");
...
<head>
  ...
  <style>
    body { background: #f8f9fa; font-family: 'Segoe UI', system-ui, sans-serif; margin: 0; padding: 0; }
...
<div class="container py-3">
  <div class="user-card">
    <div class="user-info">
      <img src="<?php echo $photo; ?>" alt="User Photo" class="user-avatar">
      <div>
        <h5 class="mb-0"><?php echo ($role === "doctor" ? "Dr. " : "") . $fullName; ?></h5>
        <small class="text-muted"><?php echo $roleLabel; ?><?php if ($role === "doctor") echo " • " . $specialization; ?></small>
      </div>
    </div>
    <div class="d-none d-md-block text-end">
      <span class="fw-bold text-primary">MediCare</span><br>
  ...
  <div class="video-container" id="root">
    ...
    <div class="call-controls">
      <button class="control-btn btn-mute" title="Mute/Unmute"><i class="fas fa-microphone"></i></button>
      <button class="control-btn btn-video" title="Video On/Off"><i class="fas fa-video"></i></button>
      <button class="control-btn btn-end" title="End Call"><i class="fas fa-phone-slash"></i></button>
      <button class="control-btn btn-chat" title="Chat"><i class="fas fa-comment-dots"></i></button>
    </div>
  </div>
</div>
...
window.onload = function () {
    const roomID = "<?php echo $roomID; ?>";  // Shared room ID
    const userID = "<?php echo $userId; ?>";
    const userName = "<?php echo $fullName . ' (' . $roleLabel . ')'; ?>";
